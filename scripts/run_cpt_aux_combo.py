...
def main():
    ...
    # data_files = {"train": "train.jsonl", "test": "validation.jsonl"}
    # raw_datasets = datasets.load_dataset("PolyAgent/ukrainian-en-wiki", data_files=data_files)
    questions = datasets.load_dataset("cais/mmlu", "auxiliary_train")['train']
    num2char = {0:"A", 1:"B", 2:"C", 3:"D"}
    prompts = []
    for row in questions['train']:
        prompts.append(f"{row['question']}\nA.{row['choices'][0]}\nB.{row['choices'][1]}\nC.{row['choices'][2]}\nD.{row['choices'][3]}\nAnswer: {num2char[row['answer']]}")
    df = pd.DataFrame(prompts)
    raw_datasets2 = datasets.Dataset.from_pandas(df.rename(columns={0:"text"}), split="train").train_test_split(test_size=0.03)
    raw_datasets2

    from datasets import concatenate_datasets

    combined_train = concatenate_datasets([raw_datasets['train'], raw_datasets2['train']]).shuffle(seed=42)
    combined_test = concatenate_datasets([raw_datasets['test'], raw_datasets2['test']]).shuffle(seed=42)
